...
def load_sharded_model(model_dir, device="cpu", safe_load=True):
    """
    Load a sharded model from a directory containing multiple safetensors files.
    Supports HuggingFace format: diffusion_pytorch_model-00001-of-00006.safetensors, etc.
    """
    import glob
    from safetensors.torch import load_file
    
    if not os.path.isdir(model_dir):
        raise ValueError(f"Expected directory for sharded model, got: {model_dir}")
    
    # Find all shard files matching pattern: *-00001-of-*.safetensors
    shard_pattern = os.path.join(model_dir, "*-[0-9][0-9][0-9][0-9][0-9]-of-*.safetensors")
    shard_files = sorted(glob.glob(shard_pattern))
    
    if not shard_files:
        raise FileNotFoundError(f"No sharded model files found in {model_dir}")
    
    log.info(f"Loading sharded model from {model_dir} ({len(shard_files)} shards)")
    
    # Load and merge all shards
    state_dict = {}
    for shard_file in tqdm(shard_files, desc="Loading shards"):
        shard_state = load_file(shard_file, device=str(device))
        state_dict.update(shard_state)
    
    log.info(f"Loaded {len(state_dict)} keys from {len(shard_files)} shards")
    return state_dict
...
class WanVideoLoraSelect:
    ...
    def getlorapath(self, lora, strength, unique_id, blocks={}, prev_lora=None, low_mem_load=False, merge_loras=True):
        ...
        try:
            ...
            # Handle sharded LoRA (directory with high_noise/low_noise safetensors)
            if os.path.isdir(lora_path):
                # For sharded LoRA, try to load metadata from the first available file
                for shard_name in ['high_noise_model.safetensors', 'low_noise_model.safetensors']:
                    shard_path = os.path.join(lora_path, shard_name)
...
class WanVideoSetLoRAs:
    ...
    def setlora(self, model, lora=None):
        ...
        for l in lora:
            ...
            # Handle sharded LoRA (directory with high_noise/low_noise safetensors)
            if os.path.isdir(lora_path):
                lora_sd = load_sharded_model(lora_path, device="cpu", safe_load=True)
            else:
...
class WanVideoModelLoader:
    ...
    def loadmodel(self, model, base_precision, load_device,  quantization,
        ...
        gguf_reader = None
        if not gguf:
            # Check if model_path is a directory (sharded model) or a file
            if os.path.isdir(model_path):
                sd = load_sharded_model(model_path, device=transformer_load_device, safe_load=True)
            else:
